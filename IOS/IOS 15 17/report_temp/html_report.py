...
def open_script_file(file_path):
...
def report_html_output(parsed_report_output, compliance_score_dict, html_filename, target_ip_address, ios_version):
    ...
    html_report = f"""
        ...
        <head>
            ...
            <title>Cisco Another Automated Assessment Tool Report</title>
            <link rel="icon" href={icon_base64()} type="image/x-icon">
            <style>
                {open_style_file("./report_temp/html_report_styles.css")}
            </style>
        </head>
        <body>
            <div class="header">
    ...
    html_report += f"""
        ...
        <script>
            {open_script_file("./report_temp/html_report_script.js")}
        </script>
        </body>
        </html>
    """
    reports_dir_check("./report_temp/reports")
    with open(f'./report_temp/reports/{html_filename}', 'w') as file:
